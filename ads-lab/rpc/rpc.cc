		...
		unsigned int xid_rep, char **b, int *sz)
{
	...
	//printf("xid_rep:%u,%u\n",xid_rep,xid);
	std::list<reply_t>& mReply = reply_window_[clt_nonce];
	for(std::list<reply_t>::iterator iter = mReply.begin();iter!=mReply.end();){
		if(iter->xid+10<xid_rep&&iter->cb_present){
			iter = mReply.erase(iter);
			continue;
		}
		iter++;
	}
	bool old = true;
	for(std::list<reply_t>::iterator iter = mReply.begin();iter!=mReply.end();iter++){
		if(iter->xid<=xid){
			old = false;
		}
		if(iter->xid==xid){  //duplicate
			if(iter->cb_present == false){
                        	return this->INPROGRESS;
			}else{
				*b = iter->buf;
				*sz = iter->sz;
				//printf("duplicate req %ld\n",xid);
				return this->DONE;
			}
		}
	}
